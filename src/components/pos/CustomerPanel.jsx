...
import { User, Users, X, ChevronDown, Gift, Star, Plus } from 'lucide-react'
...
const VIP_CONFIG = {
  ...
  GOLD:     { label: 'Gold',     cls: 'text-amber-700 bg-amber-50 border-amber-200' },
...
export function VipBadge({ tier }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[12px] font-bold whitespace-nowrap ${cfg.cls}`}>
      {cfg.label}
    </span>
  )
}
...
function CustomerSelector({ customers, selected, onSelect }) {
  ...
  return (
    <div className="relative flex-1 min-w-0">
      <button
        onClick={() => setOpen(v => !v)}
        className={`w-full flex items-center gap-2 h-9 px-3 rounded-xl border text-sm text-left transition-all ${
          selected
            ? 'bg-violet-50 border-violet-200 text-gray-900'
            : 'bg-surface2 border-gray-300 text-gray-500 hover:border-gray-400'
        }`}
      >
        ...
        {selected && (
          <span
            ...
            className="text-gray-400 hover:text-rose-500 p-0.5 rounded hover:bg-rose-50 transition-colors"
          ><X size={13} strokeWidth={2.2} /></span>
        )}
        <ChevronDown size={14} strokeWidth={2.2} className={`text-gray-400 transition-transform shrink-0 ${open ? 'rotate-180' : ''}`} />
      </button>

      {open && (
        <div className="absolute left-0 right-0 top-full mt-1.5 bg-white border border-gray-200 rounded-2xl shadow-lg z-30 overflow-hidden">
          <div className="p-2 border-b border-gray-200">
            <input autoFocus
              className="w-full bg-white border border-gray-300 rounded-lg px-3 py-1.5 text-sm text-gray-900 placeholder:text-gray-400 outline-none focus:border-cblue focus:ring-4 focus:ring-cblue/10 transition-all"
          ...
          <div className="max-h-52 overflow-y-auto">
            <button
              ...
              className="w-full px-3 py-2.5 text-left text-sm text-gray-500 hover:bg-blue-50 transition-colors flex items-center gap-2 border-b border-gray-200"
            >
            ...
            {filtered.map(c => (
              <button key={c.id}
                ...
                className={`w-full px-3 py-2.5 text-left transition-colors flex items-center justify-between gap-2 border-b border-gray-200 last:border-0 ${
                  selected?.id === c.id ? 'bg-blue-100 border-l-2 border-l-blue-300' : 'hover:bg-blue-50'
                }`}
              >
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    {c.phone && <div className="text-[12px] text-gray-500">{fmtPhone(c.phone)}</div>}
                  </div>
                </div>
            ...
            {filtered.length === 0 && (
              <div className="flex flex-col items-center justify-center gap-1.5 py-6 text-center">
                ...
                <div className="text-sm font-semibold text-gray-500">Không tìm thấy khách hàng</div>
              </div>
            )}
          </div>
        </div>
...
export default function CustomerPanel({ customers, customer, onSelect, onAddNew, onOpenRedeem }) {
  return (
    <div className="shrink-0 px-5 pt-3 pb-3 border-b border-slate-800">
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-[12px] text-slate-500 font-bold uppercase tracking-widest">Khách hàng</span>
        {customer && (
          <div className="flex items-center gap-1.5">
            <VipBadge tier={customer.vipTier || 'MEMBER'} />
            <span className="text-[12px] text-amber-700 font-bold flex items-center gap-0.5">
      ...
      <div className="flex items-center gap-2">
        ...
        {onAddNew && (
          <button
            ...
            className="shrink-0 w-9 h-9 rounded-xl border border-slate-800 bg-white text-slate-500 hover:border-cblue hover:text-cblue transition-colors flex items-center justify-center"
          >
      ...
      {customer && (customer.rewardPoints ?? 0) > 0 && (
        <button
          ...
          className="mt-2 w-full flex items-center justify-center gap-1.5 h-8 rounded-xl border border-amber-200 bg-amber-50 text-amber-700 text-[12px] font-bold hover:bg-amber-100 transition-colors"
        >
