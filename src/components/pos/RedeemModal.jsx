      ...
      <div className="bg-white border border-slate-800 rounded-2xl w-full max-w-sm mx-4 shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="px-5 py-4 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="font-bold text-[#111827]">Đổi Điểm Lấy Quà</div>
              <div className="text-xs text-slate-500 mt-0.5">Điểm hiện có: <strong className="text-amber-700">{available.toLocaleString('vi-VN')} điểm</strong></div>
            </div>
          </div>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-xl border text-left transition-all
                  ${isSelected
                    ? 'bg-amber-50 border-amber-300 text-[#111827]'
                    : canAfford
                    ? 'bg-surface2 border-slate-800 hover:border-slate-600 text-[#111827]'
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className={`text-[12px] font-bold ${canAfford ? 'text-amber-700' : 'text-slate-400'}`}>
        ...
        <div className="px-4 pb-4 flex gap-2">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-slate-800 text-slate-500 text-sm hover:text-[#111827] transition-colors">
            Huỷ
          </button>
          <button
