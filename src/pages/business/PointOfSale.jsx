  ...
  return (
    <ModalOverlay onClose={onSkip} className="bg-black/80">
      <div className="bg-surface border border-cgreen/30 rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden">
        ...
        {/* Header thành công */}
        <div className="bg-cgreen/10 px-5 py-4 text-center border-b border-cgreen/20">
        ...
        <div className="px-5 pt-3 pb-2 flex flex-col gap-2">
          ...
          <div className="flex justify-between text-sm border-t border-border pt-2 mt-1">
            <span className="text-muted font-semibold">Tổng cộng</span>
            <span className="font-black text-lg text-[#e6edf3] tabular-nums">{fmtVNDFull(total)}</span>
          </div>
          {paidAmount !== undefined && (
            <div className="flex justify-between text-sm">
              <span className="text-muted">Khách trả</span>
          ...
          {debtAmount > 0 && (
            <div className="flex justify-between text-sm rounded-lg bg-cred/10 border border-cred/25 px-3 py-2 -mx-1">
              <span className="text-cred font-bold">💳 Còn nợ</span>
              <span className="font-black text-cred tabular-nums">{fmtVNDFull(debtAmount)}</span>
            </div>
          )}
          {surplus > 0 && (
            <div className="flex justify-between text-sm rounded-lg bg-cgreen/10 border border-cgreen/25 px-3 py-2 -mx-1">
        ...
        <div className="px-5 py-3 border-t border-border">
          <div className="text-xs text-muted mb-2 font-semibold uppercase tracking-wider">Khổ giấy</div>
          <div className="flex rounded-lg overflow-hidden border border-border">
            <button
              onClick={() => setMode('thermal')}
              className={`flex-1 py-2 text-sm font-bold transition-colors flex items-center justify-center gap-1.5
                ${mode === 'thermal' ? 'bg-cblue text-white' : 'bg-surface2 text-muted hover:text-[#e6edf3]'}`}
            >
              🖨️ <span>Nhiệt 80mm</span>
            </button>
            <button
              onClick={() => setMode('a4')}
              className={`flex-1 py-2 text-sm font-bold transition-colors flex items-center justify-center gap-1.5
                ${mode === 'a4' ? 'bg-cblue text-white' : 'bg-surface2 text-muted hover:text-[#e6edf3]'}`}
            >
              📄 <span>A5 PDF</span>
            </button>
          </div>
          {mode === 'thermal' && (
            <div className="text-[10.5px] text-muted mt-1.5 text-center">
        ...
        <div className="px-5 pb-5 flex gap-3">
          ...
          <button
            ...
            className="flex-1 py-3 rounded-xl bg-cblue hover:brightness-110 text-white text-sm font-bold transition-all flex items-center justify-center gap-2 shadow-lg shadow-cblue/20 active:scale-95 touch-manipulation"
          >
